...
class QuizShuffler:
    ...
    def _shuffle_questions_list(self, questions: List[Dict]) -> None:
        ...
        logger.info(f"Original question order: {[q.get('question', '')[:20] for q in questions[:3] if 'question' in q]}...")
            
        # Force shuffle multiple times to ensure randomness
        for _ in range(3):
            random.shuffle(questions)
            
        logger.info(f"New question order: {[q.get('question', '')[:20] for q in questions[:3] if 'question' in q]}...")
    ...
    def _shuffle_options_in_questions(self, questions: List[Dict]) -> None:
        ...
        for i, question in enumerate(questions):
            try:
                ...
                logger.debug(f"Q{i} original: A={question['option_a'][:10]}, correct={question['correct_answer']}")
                ...
                for j, option in enumerate(options):
                    if option['text'] == correct_option_text:
                        ...
                        logger.debug(f"Q{i} shuffled: correct now {new_letter}")
                        break
